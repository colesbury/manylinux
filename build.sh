...
if [ "${MANYLINUX_BUILD_FRONTEND}" == "docker" ]; then
	...
	buildctl build \
		...
		--opt build-arg:PYTHON_VERSION=${PYTHON_VERSION} --opt build-arg:COMMIT_SHA=${COMMIT_SHA} \
		--output type=docker,name=nogil/${POLICY}_${PLATFORM}:${COMMIT_SHA} | docker load
else
	echo "Unsupported build frontend: '${MANYLINUX_BUILD_FRONTEND}'"
	exit 1
fi

docker run --rm -v $(pwd)/tests:/tests:ro nogil/${POLICY}_${PLATFORM}:${COMMIT_SHA} /tests/run_tests.sh
